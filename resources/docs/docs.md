...
The `home-routes` handler in the `iix.se-luminous.routes.home` namespace
...
The `home-page` function will in turn call the `iix.se-luminous.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `iix.se-luminous.handler` namespace:
...
Request middleware functions are located under the `iix.se-luminous.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `iix.se-luminous.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `iix.se-luminous.db.core` in some other namespace.
